...
.run(function ($templateCache) {
    ...
    $templateCache.put('forceHorse/buttons', '<div class="buttonsWrapper">\
...
.directive('forceHorse', ['$compile', 'ForceHorseFactory', 'ForceHorseHelper', function ($compile, ForceHorseFactory, helper) {
    return {
        ...
        controller: function controller($scope, $element) {
            ...
            this.options.forceHorseInstance = $scope.forceHorseInstance = new ForceHorseFactory($element, this.options, this.requestDigest).redraw();
 ...
 */
.factory('ForceHorseFactory', ['$http', '$log', '$timeout', 'ForceHorseConstants', 'ForceHorseHelper', function ($http, $log, $timeout, constants, helper) {
    /**
     * @ngdoc method
    ...
    proto.redraw = function () {
        ...
        $http.get(constants.CONFIG_FILE_NAME).then(function (response) {
    ...
    proto.draw = function () {
        ...
        // draw edges
        this.elements[constants.EDGES] = this.edgeGroup.selectAll("." + constants.CSS_CLASS_EDGE).data(this.edgeDataArray).enter().append("line").attr("class", constants.CSS_CLASS_EDGE).attr("stroke", function (d) {
            return d.color;
        ...
        // Draw progress bar
        this.progressBar = this.svg.append('line').attr('class', 'progress').attr('x1', '0').attr('y1', '1').attr('x2', '0').attr('y2', '1');
        ...
        for (var itemType = constants.NODES; itemType <= constants.EDGES; itemType++) {
            this.elements[itemType].filter(function (item) {
                ...
                if (itemType == constants.EDGES) {
                    _this2.decrementNodesWeightsForFilteredEdge(item);
                }
            }).classed('filtered', true).classed('selected', false);
            this.selectedItems[itemType].clear();
        }

        // Remove the labels of filtered nodes
        this.labels.classed("selected", "false").classed("filtered", function (associatedNode) {
            return associatedNode.filtered;
        });

        // Remove edges connected to filtered nodes
        this.elements[constants.EDGES].filter(function (edge) {
            return edge.source.filtered || edge.target.filtered;
        }).each(function (edge) {
            edge.filtered = true;
            _this2.decrementNodesWeightsForFilteredEdge(edge);
    ...
    proto.onFilterOutside = function () {
        ...
        for (var itemType = constants.NODES; itemType <= constants.EDGES; itemType++) {
            ...
            }).classed('filtered', true).classed('selected', false).each(function (item) {
                var type = item.class === constants.CLASS_NODE ? constants.NODES : constants.EDGES;
                myInstance.selectedItems[type].delete(item.id);
                if (type === constants.EDGES) {
                    _this3.decrementNodesWeightsForFilteredEdge(item);
                }
            });
        }
        ...
        this.elements[constants.EDGES].filter(function (edge) {
            ...
            edge.filtered = true;
            _this3.decrementNodesWeightsForFilteredEdge(edge);
        }).classed("filtered", true);

        // Update visual selection mode
        myInstance.svg.classed("selectionMode", myInstance.selectedItems[constants.NODES].size + myInstance.selectedItems[constants.EDGES].size);
    ...
    proto.onWindowResize = function () {
        return this.calcFixAspectRatio().updateGraphInDOM();
    };
    ...
    proto.onSelectInside = function (element, item, on, clearOldSelection) {
        ...
        this.svg.classed("selectionMode", this.selectedItems[constants.NODES].size + myInstance.selectedItems[constants.EDGES].size);
    ...
    proto.onSelectOutside = function () {
        ...
        for (var itemType = constants.NODES; itemType <= constants.EDGES; itemType++) {
            (function (mySet) {
                mySet.clear();
                myInstance.elements[itemType].classed('selected', function (d) {
                    if (d.selected) {
                        mySet.add(d.id);
                        return true;
                    } else {
    ...
    proto.onEdgeWeightShowHideBtnClick = function () {
        ...
        this.elements[constants.EDGES].attr("stroke-width", !this.config.showEdgeWeight ? null : function (d) {
            return myInstance.getEdgeWidth(d);
        });
        return this;
    };
    ...
    proto.onEdgesSelectedWeightLevelChange = function () {
        ...
        if (this.edgesFilteredByWeight.currentWeightLevel < this.edgesFilteredByWeight.selectedWeightLevel) {
            // filter some edges
            this.elements[constants.EDGES].filter(function (edge) {
                return edge.weight >= _this4.edgesFilteredByWeight.currentWeightLevel && edge.weight < _this4.edgesFilteredByWeight.selectedWeightLevel;
            }).each(function (edge) {
                edge.filteredByWeight = true;
                _this4.decrementNodesWeightsForFilteredEdge(edge);
            }).classed("filtered-low-weight", true);
        } else {
            // un-filter some edges
            this.elements[constants.EDGES].filter(function (edge) {
                return edge.weight >= _this4.edgesFilteredByWeight.selectedWeightLevel && edge.weight < _this4.edgesFilteredByWeight.currentWeightLevel;
            }).each(function (edge) {
                edge.filteredByWeight = false;
                _this4.incrementNodesWeightsForEdge(edge);
     ...
     * @returns {ForceHorseFactory} current instance
     */
    proto.callEventListeners = function (type) {
...
.service('ForceHorseHelper', ['$templateCache', '$compile', '$log', '$window', 'ForceHorseConstants', function ($templateCache, $compile, $log, $window, constants) {
    return {
         ...
         */
        calcRightAngledOffset: function calcRightAngledOffset(basicOffset, origDx, origDy) {
            var dx, dy;
            if (basicOffset === 0) {
                dx = dy = 0;
         ...
         * @returns {number}
         */
        computeFrictionParameter: function computeFrictionParameter(width_in_pixels, height_in_pixels, number_of_nodes) {
            var A = 0.0356,
                B = 1.162,
         ...
         * @returns {boolean}
         */
        isHebrewString: function isHebrewString(s) {
            var c = s.charAt(0);
            return c >= 'א' && c <= 'ת';
        },
        ...
        convertFileDataFormat: function convertFileDataFormat(fileData) {
            ...
            nodes.forEach(function (node, idx) {
                ...
                node.shape = _this5.getShape(node.shape);
            });
            // Process edges
...
//# sourceMappingURL=forceHorse-es5.js.map