 ...
 */
.factory('ForceHorseFactory', ['$http', '$log', 'ForceHorseConstants', 'ForceHorseHelper', function ($http, $log, constants, helper) {
    /**
     * @ngdoc method
    ...
    proto.getNodeIconArea = function (nodeData) {
        var myInstance = this;
        return myInstance.nodeIconAreaDefault + (myInstance.config.showNodeWeight ? (myInstance.config.useEdgesWeights ? nodeData.edgesWeight : angular.isDefined(nodeData.weight) ? nodeData.weight : 1) * constants.node_size_addition_per_weight_unit : 0);
    };
    ...
    proto.onFilterInside = function () {
        ...
        this.labels.classed("selected", "false").classed("filtered", function (d) {
            return d.filtered;
        });
    ...
    proto.onFilterOutside = function () {
        ...
        for (var itemType = constants.NODES; itemType <= constants.EDGES; itemType++) {
            ...
            }).classed('filtered', true).classed('selected', false).each(function (d) {
                var type = d.class === constants.CLASS_NODE ? constants.NODES : constants.EDGES;
